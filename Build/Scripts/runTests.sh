...
#
# goldene-zeiten/products test runner based on docker/podman.
#
if [ "${CI}" != "true" ]; then
...
case ${TEST_SUITE} in
    ...
    functional)
        ...
        # Start the UPS rating mock (Stoplight Prism, built from Build/mocks) so the UPS integration test
        # exercises the real HTTP path. waitFor gates on the port so the tests never race the container;
        # its URL is passed to the test container, which reaches it by name on the shared network.
        ${CONTAINER_BIN} build -t products-ups-rating-mock:runtests "${ROOT_DIR}/Build/mocks/ups-rating" >/dev/null
        ${CONTAINER_BIN} run --name ups-mock-${SUFFIX} --network ${NETWORK} -d products-ups-rating-mock:runtests >/dev/null
        waitFor ups-mock-${SUFFIX} 4010
        UPS_MOCK_PARAMS="-e UPS_MOCK_BASE_URL=http://ups-mock-${SUFFIX}:4010"
        case ${DBMS} in
            mariadb)
                echo "Using driver: ${DATABASE_DRIVER}"
                ...
                ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${CONTAINERPARAMS} ${UPS_MOCK_PARAMS} ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
            mysql)
                echo "Using driver: ${DATABASE_DRIVER}"
