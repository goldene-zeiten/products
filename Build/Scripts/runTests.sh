...
case ${TEST_SUITE} in
    ...
    functional)
        ...
        # Start the shared API mock (WireMock, mappings under Build/mocks) so functional tests exercise
        # the real HTTP path against it. waitFor gates on readiness so tests never race the container; its
        # URL is passed to the test container, which reaches it by name on the shared network. Each API is
        # a folder of stub mappings - see Build/mocks/README.md.
        ${CONTAINER_BIN} run --name mock-${SUFFIX} --network ${NETWORK} -d -v ${ROOT_DIR}/Build/mocks/wiremock:/home/wiremock:ro ${IMAGE_WIREMOCK} >/dev/null
        waitFor mock-${SUFFIX} 8080
        MOCK_PARAMS="-e MOCK_BASE_URL=http://mock-${SUFFIX}:8080"
        case ${DBMS} in
            mariadb)
                echo "Using driver: ${DATABASE_DRIVER}"
                ...
                ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${CONTAINERPARAMS} ${MOCK_PARAMS} ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
            mysql)
                echo "Using driver: ${DATABASE_DRIVER}"
