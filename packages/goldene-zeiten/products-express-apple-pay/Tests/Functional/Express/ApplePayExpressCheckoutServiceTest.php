...
namespace GoldeneZeiten\Products\Express\ApplePay\Tests\Functional\Express;
...
/**
 * The confirm orchestration end to end: recompute the amount from the shop's basket, authorize the Apple
 ...
 */
final class ApplePayExpressCheckoutServiceTest extends AbstractApplePayExpressMockTestCase
{
    protected function setUp(): void
    {
        parent::setUp();
        $this->importCSVDataSet(__DIR__ . '/../../../../products-core/Tests/Functional/Service/Order/Fixtures/order_placement.csv');
        $GLOBALS['TYPO3_CONF_VARS']['EXTENSIONS']['products_express_apple_pay'] = [
            'merchantIdentifier' => 'merchant.com.test',
            'displayName' => 'Test Shop',
            'countryCode' => 'DE',
            'apiBaseUrl' => $this->mockRoot . '/express/apple',
            'apiKey' => 'proc-key',
        ];
    }
    ...
    {
        $order = $this->get(ApplePayExpressCheckoutService::class)->confirm(
            $this->request(),
            ...
            '',
            ['transactionIdentifier' => 'APPROVE']
        );

        $this->assertSame(PaymentStatus::PAID, $order->getPaymentStatus());
        $this->assertSame('apple-pay-express', $order->getPaymentMethod());
    ...
    public function aDeclinedTokenCreatesNoOrder(): void
    {
        ...
        $this->expectExceptionCode(1784220846);

        $this->get(ApplePayExpressCheckoutService::class)->confirm(
            $this->request(),
            $this->basketViewModel($this->product()),
            new Address(country: 'DE'),
            '',
            ['transactionIdentifier' => 'DECLINE']
        );
    }
