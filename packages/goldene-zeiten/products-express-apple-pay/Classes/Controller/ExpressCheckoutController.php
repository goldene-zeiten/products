...
namespace GoldeneZeiten\Products\Express\ApplePay\Controller;
...
use GoldeneZeiten\Products\Express\ApplePay\Express\ApplePayExpressCheckoutProvider;
use GoldeneZeiten\Products\Express\ApplePay\Express\ApplePayExpressCheckoutService;
...
/**
 * The frontend seam of the Apple Pay express checkout. {@see buttonAction} renders the button and hands its
 * JS the signed basket token; {@see validateAction} answers the sheet's `onvalidatemerchant` callback by
 * validating the merchant session through the processor; and {@see confirmAction} authorizes the token and
 * creates the paid order once the buyer has authorized, answering with the thank-you URL.
 *
 * The two JSON actions run as their own typeNum PAGEs so the Apple Pay JS receives raw JSON. Live shipping
 * has no action here - the sheet's shipping callbacks are answered client-side from the core shipping-quote
 * endpoint.
 */
final class ExpressCheckoutController extends ActionController
{
    public function __construct(
        ...
        private readonly ApplePayExpressCheckoutService $checkoutService
    ) {}
    ...
    public function buttonAction(): ResponseInterface
    {
        ...
        $provider = $this->providerRegistry->get(ApplePayExpressCheckoutProvider::IDENTIFIER);
    ...
    {
        $validationUrl = (string)($this->jsonBody()['validationURL'] ?? '');
        if ($validationUrl === '') {
            return new JsonResponse(['error' => 'missing_validation_url'], 400);
        }
        try {
            $session = $this->checkoutService->validateMerchant($validationUrl, $this->request->getUri()->getHost());
        } catch (ApplePayProcessorException) {
            return new JsonResponse(['error' => 'validation_failed'], 502);
        }
    ...
    public function confirmAction(): ResponseInterface
    {
        ...
        try {
            $order = $this->checkoutService->confirm(
                ...
                is_array($body['token'] ?? null) ? $body['token'] : []
            );
