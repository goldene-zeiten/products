...
async function fillAddressAndReachShippingStep(page, email) {
  ...
  await page.locator('input[name="tx_products_checkout[address][email]"]').fill(email);
  await page.locator('input[name="tx_products_checkout[address][firstName]"]').fill('Del');
  await page.locator('input[name="tx_products_checkout[address][lastName]"]').fill('Ivery');
  await page.locator('input[name="tx_products_checkout[address][street]"]').fill('Some Street 3');
  await page.locator('input[name="tx_products_checkout[address][zip]"]').fill('33333');
  await page.locator('input[name="tx_products_checkout[address][city]"]').fill('Frankfurt');
