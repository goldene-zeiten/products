...
test('cancelling at the shipping method step leaves the basket untouched', async ({ page }) => {
  await addProductAndGoToCheckout(page);
  await page.locator('input[name="tx_products_checkout[address][email]"]').fill('cancel1@example.com');
  await page.locator('input[name="tx_products_checkout[address][firstName]"]').fill('Can');
  await page.locator('input[name="tx_products_checkout[address][lastName]"]').fill('Cel');
  await page.locator('input[name="tx_products_checkout[address][street]"]').fill('Street 1');
...
test('cancelling at the payment step leaves the basket untouched', async ({ page }) => {
  await addProductAndGoToCheckout(page);
  await page.locator('input[name="tx_products_checkout[address][email]"]').fill('cancel2@example.com');
  await page.locator('input[name="tx_products_checkout[address][firstName]"]').fill('Can');
  await page.locator('input[name="tx_products_checkout[address][lastName]"]').fill('Cel');
  await page.locator('input[name="tx_products_checkout[address][street]"]').fill('Street 1');
  await page.locator('input[name="tx_products_checkout[address][zip]"]').fill('11111');
...
test('cancelling at the review step leaves the basket untouched and places no order', async ({ page }) => {
  await addProductAndGoToCheckout(page);
  await page.locator('input[name="tx_products_checkout[address][email]"]').fill('cancel3@example.com');
  await page.locator('input[name="tx_products_checkout[address][firstName]"]').fill('Can');
  await page.locator('input[name="tx_products_checkout[address][lastName]"]').fill('Cel');
  await page.locator('input[name="tx_products_checkout[address][street]"]').fill('Street 1');
  await page.locator('input[name="tx_products_checkout[address][zip]"]').fill('11111');
