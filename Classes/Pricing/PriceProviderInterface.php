...
/**
 * Resolves the unit price a basket line should use. Bound to `UserGroupDiscountPriceProvider` by
 * default (see Services.yaml), which decorates `GraduatedPriceProvider` as its own fallback rather
 * than as an alternative binding, so quantity-based pricing is always considered before any
 * per-shopper discount is applied on top.
 */
